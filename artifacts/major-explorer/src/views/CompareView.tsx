...
import { Scale, GraduationCap, MapPin, ArrowLeftRight, TrendingUp } from "lucide-react";
    ...
    <TiltedCard>
      ...
      <div className="glass-panel rounded-2xl overflow-hidden h-full">
        ...
        <div className="px-5 md:px-6 py-4 space-y-4">
          ...
          {data.career && (
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-1.5">Career outlook</p>
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-background rounded-xl px-2 py-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground font-medium mb-0.5">Median pay</p>
                  <p className="text-sm font-bold text-foreground" data-testid={`text-compare-wage-${data.majorName}`}>{formatUSD(data.career.annualMedianWage)}</p>
                </div>
                <div className="bg-background rounded-xl px-2 py-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground font-medium mb-0.5">Job growth</p>
                  <p className={`text-sm font-bold inline-flex items-center justify-center gap-1 ${data.career.projectedGrowthPct >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`} data-testid={`text-compare-growth-${data.majorName}`}>
                    <TrendingUp className="w-3 h-3" />
                    {data.career.projectedGrowthPct >= 0 ? "+" : ""}{data.career.projectedGrowthPct}%
                  </p>
                </div>
                <div className="bg-background rounded-xl px-2 py-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground font-medium mb-0.5">Education</p>
                  <p className="text-xs font-semibold text-foreground leading-tight">{data.career.typicalEducation}</p>
                </div>
              </div>
              <p className="text-[11px] text-muted-foreground mt-1.5">Typical career: {data.career.occupation}</p>
            </div>
          )}
          {mix.length > 0 && (
            <div>
