...
- **Legal pages** — `/privacy` and `/terms` (wouter routes, public), content in `src/pages/legal/`. Linked from the landing-page footer and Settings → Data & privacy. `LEGAL_CONTACT_EMAIL` in `LegalLayout.tsx` is a placeholder (null) — set a real contact email before app-store/school distribution.
